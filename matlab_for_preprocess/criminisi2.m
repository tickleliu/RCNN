function [inpaintedImg,origImg,fillImg,C,D] = inpaint7(imgFilename,fillFilename,fillColor)
...
Iy = sum(Iy,3)/(3*255);
Iy(~isfinite(Iy))=0;
Ix1 = Ix./(255*sqrt(Ix.^2 + Iy.^2));
Iy1 = Iy./(255*sqrt(Ix.^2 + Iy.^2));
temp = Ix1; Ix1 = -Iy1; Iy1 = temp; % Rotate gradient 90 degrees梯度旋转90度即为等照度线方向
Ix = Ix1;
Iy = Iy1;
...
while any(fillRegion(:))
 ...
 % robust algorithm for exemplar based image inpainting
  C(Hp(toFill))  = 0.3 * C(p) + 0.7;%被去掉那块的修复块的置信度被更新，C代表sourceRegion，待修复点为0，已存在点为1；用优先权最大的修复点的置信度来代替优先权最大的块的置信度
...
% fillRegion = fillImg(:,:,1)==fillColor(1) & ...
%     fillImg(:,:,2)==fillColor(2) & fillImg(:,:,3)==fillColor(3);
fillRegion = fillImg(:,:,1)~=0 | ...
    fillImg(:,:,2)~=0 | fillImg(:,:,3)~=0;
% fillRegion = fillImg(:,:,1)==fillColor(1) & ...
%     fillImg(:,:,2)==fillColor(2) & fillImg(:,:,3)==fillColor(3);
